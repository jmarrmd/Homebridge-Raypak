...
import { RaypakApi, HeaterDevice, HeaterStatus } from './raypakApi';

const DEFAULT_POLL_INTERVAL_MINUTES = 1;
const FAHRENHEIT_TO_CELSIUS = (f: number): number => (f - 32) * 5 / 9;
const CELSIUS_TO_FAHRENHEIT = (c: number): number => c * 9 / 5 + 32;

export class RaypakHeaterAccessory {
  private readonly heaterService: Service;
  private readonly pollIntervalMs: number;

  private lastStatus: HeaterStatus | null = null;

  constructor(
    private readonly platform: RaypakPlatform,
    private readonly accessory: PlatformAccessory,
    private readonly apiClient: RaypakApi,
    private readonly device: HeaterDevice,
  ) {
  ...
  private async handleTargetTempSet(value: CharacteristicValue): Promise<void> {
    const tempF = Math.round(CELSIUS_TO_FAHRENHEIT(value as number));
  ...
  private async poll(): Promise<void> {
    ...
    this.heaterService.updateCharacteristic(
      Char.Active,
      status.isOn ? Char.Active.ACTIVE : Char.Active.INACTIVE,
    );

    this.heaterService.updateCharacteristic(
      Char.CurrentHeaterCoolerState,
      !status.isOn
    ...
    this.heaterService.updateCharacteristic(
      ...
      FAHRENHEIT_TO_CELSIUS(status.currentTemp),
    );

    this.heaterService.updateCharacteristic(
      Char.HeatingThresholdTemperature,
      FAHRENHEIT_TO_CELSIUS(status.targetTemp),
    );
